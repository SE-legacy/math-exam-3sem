...
#theorem[
	...
	$
		...
		limits(integral.double)_Phi P dif y dif z + Q dif z dif x + R dif x dif y = limits(integral.triple)_V (frac(partial P, partial x) + frac(partial Q, partial y) + frac(partial R, partial z)) dif x dif y dif z
	$
][
	Докажем
	$ limits(integral.double)_Phi R(x, y, z) dif x dif y = limits(integral.triple)_V frac(partial R, partial z) dif x dif y dif z. $
	Для правильной в направлении оси $O z$ замкнутой области $V$:
	$
	limits(integral.triple)_V frac(partial R, partial z) dif x dif y dif z = limits(integral.double)_D_(x y) d x d y limits(integral)_(phi_1(x, y))^(phi_2(x,y)) frac(partial R, partial z) dif z = \
	...
	$ limits(integral.double)_Phi_k R(x, y, z) dif x dif y = limits(integral.triple)_V_k frac(partial R, partial z) dif x dif y dif z, k = overline(1\,m), $
	причём
	$ limits(sum)_(k = 1)^m limits(integral.double)_Phi_k R(x, y, z) dif x dif y = limits(sum)_(k = 1)^m limits(integral.triple)_V_k frac(partial R, partial z) dif x dif y dif z = limits(integral.triple)_V frac(partial R, partial z) dif x dif y dif z $
