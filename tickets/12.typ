...
Пусть $f in Re_([-pi, pi])$ и $f$ --- чётная. Тогда $forall n in NN$
#equ($
...
Аналогично, пусть $f in Re_([-pi, pi])$ и $f$ --- нечётная. Тогда $forall n in NN$
#equ($
...
Таким образом, ряд Фурье чётной функции не содержит косинусов кратных дуг, то есть
$
...
Пусть функция $f$ определена на отрезке $[-l, l]$. Положим $x(t) = (l t)/pi$. Эта функция осуществляет взаимно однозначное отображение отрезка $[-pi, pi]$ на $[-l, l]$. Поэтому функция $phi(t) = f((l t)/pi)$ определена на отрезке $[-pi, pi]$, и
