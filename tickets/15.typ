...
#theorem(
  ...
  Если функция $f$ $2pi$-периодическая и непрерывная, то она равномерно непрерывная и ограниченная. Пусть $epsilon > 0$. Тогда найдется $delta > 0$ такое, что при всех $x in RR$ выполняется неравенство
  ...
  Найдется номер $n_epsilon$ такой, что при всех $n > n_epsilon$ выполняется неравенство

  $ (2 M)/((n + 1) sin^2 delta/2) < epsilon/2. $

  Поэтому при всех $n > n_epsilon$ и для любых $x in RR$ выполняется неравенство
