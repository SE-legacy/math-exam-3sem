...
#theorem(
  ...
  Если функция $f$ определена и непрерывна на отрезке $[a,b]$, то для любого $epsilon > 0$ найдется алгебраический многочлен $P_epsilon$ такой, что при всех $x in [a,b]$ выполняется неравенство $abs(f(x) - P_epsilon (x)) < epsilon$.
][
  Построим функцию
  ...
  Очевидно, что $2 pi$-периодическая функция $phi (t)$ непрерывна на всей числовой прямой. Мы можем воспользоваться второй теоремой Вейерштрасса.

  Пусть $epsilon > 0$. Тогда найдется тригонометрический многочлен $T_epsilon$ такой, что для всех $t in RR$ выполняется неравенство
  ...
  с бесконечным радиусом сходимости. На отрезке $[0, pi]$ (как и на любом другом отрезке) ряд сходится равномерно. Поэтому найдется номер $n in NN$, что при всех $t in [0, pi]$ выполняется неравенство
  ...
  справедливую для всех $t in [0, pi]$. Положим теперь
  ...
  то при всех $x in [a, b]$ окончательно имеем неравенство

  $ abs(f (x) - P_epsilon (x)) < epsilon. $
]
